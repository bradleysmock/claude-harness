...
def _exec(command: list[str], env: TypeScriptEnv, timeout: int = 60) -> ProcessResult:
    p = subprocess.run(
...
def _rel(path: str, env: TypeScriptEnv) -> str:
    try:
...
def _type_check_gate(env: TypeScriptEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec(["npx", "--yes", "tsc", "--noEmit", "--pretty", "false"], env)
...
def _lint_gate(env: TypeScriptEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec([
            "npx", "--yes", "eslint",
        ...
        for file_result in lint_results:
            for msg in file_result.get("messages", []):
                ...
                errors.append(GateError(
                    ...
                    file=_rel(file_result.get("filePath", ""), env),
...
def _test_gate(env: TypeScriptEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec([
            "npx", "--yes", "jest",
...
def run_typescript_suite(
    ...
    results = []
    try:
        for gate_fn in [_type_check_gate, _lint_gate, _test_gate]:
            result = gate_fn(env)
