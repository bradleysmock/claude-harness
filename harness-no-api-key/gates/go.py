...
def _exec(command: list[str], env: GoEnv, timeout: int = 60) -> ProcessResult:
    p = subprocess.run(
...
def _build_gate(env: GoEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec(["go", "build", "./..."], env)
...
def _vet_gate(env: GoEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec(["go", "vet", "./..."], env)
    except subprocess.TimeoutExpired:
...
def _test_gate(env: GoEnv) -> GateResult:
    start = time.monotonic()
    try:
        result = _exec(["go", "test", "-race", "-v", "./..."], env, timeout=120)
    except subprocess.TimeoutExpired:
    ...
    for line in result.output.splitlines():
        if line.startswith("--- FAIL:"):
            ...
            errors.append(GateError(
                ...
                file="implementation_test.go", line=None, column=None,
    ...
    if not errors:
        ...
        errors = compile_errs or [GateError(
            message=result.output[:600], file="implementation.go",
