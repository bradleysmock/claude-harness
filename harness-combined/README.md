...
| `critique` | "critique my changes", "expert panel review of the auth route" | Free-form expert-panel critique of a chosen scope — pass files/globs/a git ref, or omit to be asked (default: the whole codebase focused on changes since the last critique). Writes a timestamped report under `.harness/critiques/`. |
