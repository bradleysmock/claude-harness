...
#: The submodule `mcp` 2.x drops. Any probe that does not reach it is the false
#: negative this ticket exists to remove.
...
#: How far above the probe line the source-of-truth comment may sit.
_COMMENT_LOOKBACK_LINES = 6
...
def test_ac2_health_check_no_longer_probes_the_bare_package() -> None:
    """Every probe must reach the submodule — a bare `import mcp` passes under 2.x."""
    bare_package_probes = [
        probe for probe in _launcher_probes() if REQUIRED_SUBMODULE not in probe
    ]
    assert bare_package_probes == []
...
def test_fr3_health_check_names_server_py_as_its_source_of_truth() -> None:
    ...
    """
    launcher_lines = LAUNCHER.read_text(encoding="utf-8").splitlines()
    probe_line_index = next(
        index for index, line in enumerate(launcher_lines) if REQUIRED_IMPORT in line
    )
    comment_block = launcher_lines[
        max(0, probe_line_index - _COMMENT_LOOKBACK_LINES) : probe_line_index
    ]
...
def _site_packages(venv: Path) -> Path:
    """The POSIX venv layout, which is the only one reachable here.

    Every hermetic test runs the real launcher, a bash script that hardcodes
    `"$VENV/bin/python"`, so a Windows `Lib/site-packages` venv could never be
    the one under test.
    """
...
def _as_text(captured_stream: bytes | str | None) -> str:
    """Decode a captured stream that may or may not already be text.

    ``TimeoutExpired`` carries the undecoded chunks on POSIX even under
    ``text=True``, while ``subprocess.run`` refills them from ``communicate()``
    — already ``str`` — on Windows.
    """
    if captured_stream is None:
        return ""
    if isinstance(captured_stream, str):
        return captured_stream
    return captured_stream.decode("utf-8", "replace")
...
@_NEEDS_BASH
def test_ac4_healthy_venv_starts_without_a_reinstall(tmp_path: Path) -> None:
...
def test_fr2_fresh_bootstrap_installs_mcp_below_two_and_starts(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Not the expected exit: `stdin=DEVNULL` gives the stdio server EOF, so it
        # returns. A server build that ignores EOF would hang here instead, and
        # reaching the hang still means the bootstrap succeeded — hence the branch,
        # and hence only the bootstrap output is examined.
        launcher_stderr = _as_text(expired.stderr)
    else:
