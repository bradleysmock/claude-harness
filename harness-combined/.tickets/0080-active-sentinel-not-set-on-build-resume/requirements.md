...
3. The existing fallback branch's `echo 'XXXX-<slug>' > .tickets/.active`
   line must be corrected to the same explicit worktree-relative path, since
   its current bare-relative form is ambiguous about cwd and, run from the
   repo root as most of this doc's other commands are, would write to the
   wrong file.
...
| Type        | Rationale                                                          |
|-------------|-----------------------------------------------------------------------|
| Doc-wiring  | Step 2's normal-resume path names the unconditional `.active` write with the correct worktree-relative path; the fallback branch's existing line is corrected to the same explicit path |
...
- The fallback branch's `.active` write uses the same explicit
  worktree-relative path as the normal-path write (no ambiguity left).
