...
Resolve `$worktree_path` from `status.md` branch or the default worktree convention. Before attempting rebase, detect an in-progress rebase. `$worktree_path` is a **linked** worktree (`.git` is a file), so rebase state lives under the resolved per-worktree git dir, not at `$worktree_path/.git/`; resolve it with `git -C "$worktree_path" rev-parse --git-path rebase-merge` (and `rebase-apply`) and check `[[ -d "$rebase_merge" || -d "$rebase_apply" ]]`. (The bare `[[ -f "$worktree_path/.git/REBASE_HEAD" ]]` form is wrong for linked worktrees — it can never exist and would fail open; corrected during build per post-build critic round 1.) If a rebase is in progress: "Worktree is already in a mid-rebase state — resolve or abort the existing rebase manually before delivering." **Halt delivery.** Use the same `$worktree_path` variable in the check and in sub-step 2b-4 to eliminate TOCTOU on the path. This guard parallels the mid-rebase check in Step 9 (which handles other in-flight worktrees); they are intentionally independent — Step 2b operates on the current ticket before merge; Step 9 operates on all other tickets after merge. Do not extract these into a shared helper.
