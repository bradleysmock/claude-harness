...
1. `autopilot-ticket.md` must set an explicit mode signal (`MODE=autopilot`)
   before delegating to `build-ticket.md`, mirroring `build-dry-run-ticket.md`'s
   `DRY_RUN=true`.
2. `build-ticket.md` Step 1's score-spec BLOCK stop must branch explicitly on the
   signal: `MODE=autopilot` → continue at `autopilot-ticket.md` Step S; any other
   value must keep today's stop-and-report-to-lead behavior.
3. `build-ticket.md` Step 7d's repair-exhaustion stop must branch explicitly on
   the signal: `MODE=autopilot` → continue at Step A; any other value must keep
   today's `changes-requested` + ask-the-lead behavior.
4. `build-ticket.md` Steps 7b/7c (clean/repaired build) must branch explicitly on
   the signal: `MODE=autopilot` → continue at Step B; any other value must keep
   today's "tell the lead, list options" behavior.
5. `autopilot-ticket.md`'s "Steps 1–7c — Build" interception prose ("Spec-BLOCK
   interception", "Divergence condition", "Clean-build interception") must be
   removed; the file must retain only the mode announcement, signal assignment,
   delegation line, and Steps S/A/B.
6. The three branch points must use an actual unit-tested Python predicate — e.g.
   ...
   preserved non-autopilot behavior (FR-2/3/4's "any other value" branch) must
   remain verbatim in `build-ticket.md` alongside its new branch.
7. `autopilot-batch.md`'s per-member override block and conditions must remain
   unchanged.

## Non-Functional Requirements

1. Backward compatibility: callers that never set `MODE=autopilot` must exhibit
   today's unchanged fail-closed stop behavior.
2. Branch text must be plain-string-matchable by a content-verification test.
