...
Date arithmetic (cycle time, ISO-week grouping, averages) is delegated to `${CLAUDE_PLUGIN_ROOT}/skills/velocity/compute.py` so the numbers are **deterministic** — the same completed-ticket set always yields the same report. LLM inference is never used for the math.
...
```
echo "$json_payload" | python3 "${CLAUDE_PLUGIN_ROOT}/skills/velocity/compute.py"
```
