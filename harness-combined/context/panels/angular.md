...
*Active when Angular is in the project: `@angular/core` in `package.json`; `angular.json` or `nx.json` (with Angular workspace) present; `**/*.component.ts`, `**/*.service.ts`, `**/*.module.ts`, `**/*.directive.ts`, `**/*.pipe.ts`, or component template files (`**/*.component.html`) in scope. TypeScript-level concerns (type discipline, async correctness, ESM/CJS) defer to the TypeScript/JS panel; this panel covers Angular-specific machinery — change detection, signals, dependency injection, RxJS lifecycle, standalone components, and the migration surface between legacy and modern Angular.*
