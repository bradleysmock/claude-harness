...
5. Re-spawn the critic subagent (**Round**: `N+1`, same Phase/Ticket) to verify. Under the fail-closed fallback, spawn it exactly as Step 7's round-1 critic (full-worktree read instruction, no marker). Otherwise, spawn it with `Mode: incremental`, the touched-files list, and the formatted brief from item 4b in its prompt **instead of** the full-worktree read instruction. Display its report verbatim. Parse the new report into `Finding` objects via `gates.critic_finding_parser.parse_critic_findings(report_text, worktree_root)` (all severities) — this is round N+1's `curr`.
